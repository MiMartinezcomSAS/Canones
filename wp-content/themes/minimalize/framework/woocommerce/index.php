		...
		echo '<div class="content-main">';
			echo '<div class="container">';
				...
				if($page_layout != 'content-full-width'):
					echo '<section id="primary" class="with-sidebar '.$page_layout.'">';
				else:
...
function dt_woocommerce_show_product_loop_sale_flash() {
	...
	if( $product->is_featured())
		$out .= '<div class="featured-tag"><div><i class="fa fa-trophy"></i><span>'.__('Featured','iamd_text_domain').'</span></div></div>';
...
function dt_woocommerce_template_loop_product_thumbnail() {
	...
	$image = !empty( $image ) ? $image : "<img src='http://placehold.it/575x545' alt='product-thumb' />";
	$attachment_ids = $product->get_gallery_attachment_ids();
...
function dt_woocommerce_after_shop_loop() {
	...
	if($wp_query->max_num_pages > 1): ?>
        <div class="pagination">
            <?php if(function_exists("dttheme_pagination")) echo dttheme_pagination("", $wp_query->max_num_pages, $wp_query); ?>
        </div><?php
	endif;
}
...
function dt_woocommerce_show_product_sale_flash() {
	...
	if( $product->is_on_sale() and $product->is_in_stock() )
		$out .= '<span class="onsale"><span>'.__('<i class="fa fa-star"></i>','woocommerce').'</span></span>';
