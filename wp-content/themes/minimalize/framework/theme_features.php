<?php if (!function_exists('dt_theme_features')) {
	...
	function dt_theme_features() {
		...
		add_image_size('portfolio-single', 573, 287, true);
		add_image_size('portfolio-single-fullwidth', 1170, 585, true);
		
		add_image_size('blog-single-fullwidth', 1160, 800, true);
		add_image_size('blog-single-with-sidebar', 870, 600, true);
		add_image_size('blog-single-with-both-sidebar', 570, 393, true);
		
		add_image_size('blog-thumb-widget', 420, 290, true);
...
if (!function_exists('dttheme_activation_function')) {
	function dttheme_activation_function($oldname, $oldtheme=false) {
		if(!in_array($oldname, array('Minimalize', 'Minimalize Child'))) {
