	...
	function widget($args, $instance) {
		...
		echo "<ul class='recent-posts-widget'>";		
			 ...
			 while(have_posts()):
				...
				$id = get_the_id();
				$post_thumbnail = 'blog-thumb-widget';
				echo "<li>";
						if( has_post_thumbnail($id) ) {
							echo '<a class="thumb" href="'.get_permalink().'" title="'.$title.'">'.get_the_post_thumbnail($id,$post_thumbnail).'</a>';
						}else{
							echo '<a class="thumb" href="'.get_permalink().'" title="'.$title.'"><img src="http://placehold.it/420x290" alt="'.$title.'" /></a>';
						}
