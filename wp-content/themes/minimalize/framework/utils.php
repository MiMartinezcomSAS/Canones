...
function dttheme_social_bookmarks($arg = 'sb-post') {
	...
	$fb = dttheme_option ( 'integration', "{$arg}-fb_like" );
	$data .= ! empty ( $fb ) ? '<li><a href="http://www.facebook.com/sharer.php?u=$url&amp;t='.urlencode($title).'"><i class="fa fa-facebook"></i></a></li>' : '';
	
	$delicious = dttheme_option ( 'integration', "{$arg}-delicious" );
	$data .= ! empty ( $delicious ) ? '<li><a href="http://del.icio.us/post?url=$url&amp;title='.urlencode($title).'"><i class="fa fa-delicious"></i></a></li>' : '';
	
	$digg = dttheme_option ( 'integration', "{$arg}-digg" );
	$data .= ! empty ( $digg ) ? '<li><a href="http://digg.com/submit?phase=2&amp;url=$url&amp;title='.urlencode($title).'"><i class="fa fa-digg"></i></a></li>' : '';
	
	$stumbleupon = dttheme_option ( 'integration', "{$arg}-stumbleupon" );
	$data .= ! empty ( $stumbleupon ) ? '<li><a href="http://www.stumbleupon.com/submit?url=$url&amp;title='.urlencode($title).'"><i class="fa fa-stumbleupon"></i></a></li>' : '';
	
	$twitter = dttheme_option ( 'integration', "{$arg}-twitter" );
	$t_url = ! empty ( $twitter ) ? $url : '';
	$data .= ! empty ( $twitter ) ? '<li><a href="http://twitter.com/home/?status='.urlencode($title).':'.$t_url.'"><i class="fa fa-twitter"></i></a></li>' : '';
	
	$googleplus = dttheme_option ( 'integration', "{$arg}-googleplus" );
	$data .= ! empty ( $googleplus ) ? '<li><a href="https://plus.google.com/share?url='.$url.'"><i class="fa fa-google-plus"></i></a></li>' : '';
	
	$linkedin = dttheme_option ( 'integration', "{$arg}-linkedin" );
	$data .= ! empty ( $linkedin ) ? '<li><a href="http://www.linkedin.com/shareArticle?mini=true&amp;title='.urlencode($title).'"><i class="fa fa-linkedin"></i></a></li>' : '';
	
	$pintrest = dttheme_option ( 'integration', "{$arg}-pintrest" );
	$media = wp_get_attachment_url ( get_post_thumbnail_id ( $post->ID ) );
	$data .= ! empty ( $pintrest ) ? '<li><a href="http://pinterest.com/pin/create/button/?url='.urlencode($url).'&amp;media='.$media.'"><i class="fa fa-pinterest"></i></a></li>' : '';
...
function dttheme_load_basic_css() {
	...
	wp_enqueue_style ( 'pushy', IAMD_BASE_URL . 'css/pushy.css' );
	
	wp_enqueue_style ( 'minimalize-default', get_stylesheet_uri () );
	...
	if(dttheme_is_plugin_active('woocommerce/woocommerce.php')){
	   wp_enqueue_style('woocommerce', IAMD_BASE_URL .'/framework/woocommerce/style.css');
	}
	...
	if(dttheme_option("general","enable-dark-layout")){
		wp_enqueue_style ( 'darkskin-css', IAMD_BASE_URL.'css/dark-skin.css');
	}else {
		wp_enqueue_style ( 'darkskin-css', '#');
	}
...
function dt_ajax_load_portfolio_posts() {
	...
	if($the_query->have_posts()): 
		...
		while($the_query->have_posts()): $the_query->the_post();
			...
			$out .= '<div id="portfolio-'.$the_id.'" class="no-space '.$temp_class.$sort.'">
					<figure>';
						...
						if( array_key_exists('items_name', $portfolio_item_meta) ) {
							...
							if( "video" === $item ) {
								...
								if( !empty($items) ) {
									$out .= '<img src="'.$portfolio_item_meta['items'][key($items)].'" width="800" height="400" alt="" />';	
								} else {
									$out .= '<img src="http://placehold.it/800x400" width="800" height="400" alt="" />';
								}
							} else {
							...
							$out .= '<img src="'.$popup.'" width="800" height="400" alt="" />';
						}
							
						$out .= '<figcaption>
									<div class="fig-content">
										<h5><a href="'.get_permalink().'" title="'.get_the_title().'">'.get_the_title().'</a></h5><br>
										<a class="dt-sc-button" href="'.get_permalink().'">'.__("VIEW project","dt_themes").' <i class="fa fa-thumbs-o-up"></i> </a>';
						$out .= '	</div>
								 </figcaption>
			...
			<article id="post-'.get_the_ID().'" class="'.$class.'">';
                    ...
                    $out .= '<div class="black-box">  
                        ...
                        <div class="entry-details">';
                            ...
                            $out .= '<div class="entry-metadata">
                                 <p class="date '.$hide_date_meta.'">
								     <i class="fa fa-calendar"></i> ';
                                     $out .= get_the_date('d M Y');
									 if($hide_author_meta != ' hidden ' || $hide_comment_meta != ' hidden ' || $hide_category_meta != ' hidden ' || $hide_tag_meta != ' hidden '  ){
					 	 				$out .= " |";
				 	  				 }
                                 $out .= '</p>
                                 <p class="author '.$hide_author_meta.'">';
                                 $out .= '<i class="fa fa-user"></i>'.__('Posted By:','dt_themes').'
                                     <a href="'.get_author_posts_url(get_the_author_meta('ID')).'" title='.__('View all posts by ', 'dt_themes').get_the_author().'">'.get_the_author().'</a>';
                                if( $hide_comment_meta != ' hidden ' || $hide_category_meta != ' hidden ' || $hide_tag_meta != ' hidden '  ){
					 	 			$out .= " |";
				 	   			}
								 $out .='</p>
                                 <p class="'.$hide_comment_meta.'">';
                                 $out .= '
									...
									if( $hide_category_meta != ' hidden ' || $hide_tag_meta != ' hidden '  ){
					 	 				$out .= " |";
				 	   				}
								 $out .='</p>';
                                  if( has_category() ) {
								 $out .='<p class="category '.$hide_category_meta.'"> ';
                                 $out .= '
									...
									if(has_tags()){
                                 $out .= '<p class="tags '.$hide_tag_meta.'"> ';
                                 $out .= '
						...
						$out .= '<div class="entry-thumb">';
								if( $format === "image" ):
									$out .= '<a href="'.get_permalink().'" title="'.get_the_title().'">';
										if( get_the_post_thumbnail(get_the_ID(), "full") != '' ):
											...
											$img_attributes = wp_get_attachment_image_src($attachment_id, "full");
											$out .= "<img src='".$img_attributes[0]."' alt=".get_the_title()." />";
										elseif($pholder != 'on'):
											$out .= '<img src="http://placehold.it/1160x800&text='.get_the_title().'" alt="'.get_the_title().'" title="'.get_the_title().'" />';
										endif;
									$out .= '</a>';
								elseif( $format === "gallery" && array_key_exists("items", $post_meta)):
										...
										foreach ( $post_meta['items'] as $item ) { 
											...
											$img_attributes = wp_get_attachment_image_src($attachment_id, "full");
											$out .= "<li><img src='".$img_attributes[0]."' /></li>";
										}
										$out .= '</ul>';
                ...
                $out .= '<div class="entry-details">
                    ...
                    $out .= '<div class="entry-metadata">
                         ...
                         <p class="author '.$hide_author_meta.'"> ';
                                 $out .= '
                             <i class="fa fa-user"></i> '.__('Posted By:','dt_themes').'
                             <a href="'.get_author_posts_url(get_the_author_meta('ID')).'" title="'.__('View all posts by ', 'dt_themes').get_the_author().'">'.get_the_author().'</a>';
                         if( $hide_comment_meta != ' hidden ' || $hide_category_meta != ' hidden ' || $hide_tag_meta != ' hidden '  ){
					 	 			$out .= " |";
				 	   			}
