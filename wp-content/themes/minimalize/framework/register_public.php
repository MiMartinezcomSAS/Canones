...
function plugin_head_styles_scripts() {
	...
	$tpl_header_styles = get_post_meta( $id, '_tpl_default_settings', TRUE );
	...
	wp_enqueue_script('scrollto-script', IAMD_FW_URL.'js/public/jquery.scrollTo.min.js',array(),false,true);
	...
	if($tpl_header_styles == 'type3'):
		wp_enqueue_script('pushy-script', IAMD_FW_URL.'js/public/pushy.min.js',array(),false,true);
		wp_enqueue_script('visualnav', IAMD_FW_URL.'js/public/jquery.visualNav.min.js',array(),false,true);
		wp_enqueue_script('slidebars', IAMD_FW_URL.'js/public/slidebars.min.js',array(),false,true);
	endif;
...
function dttheme_appearance_css() {
	...
	if (empty($disable_menu)) :
		...
		$output .= "#main-menu ul li ul.sub-menu li:after, #main-menu ul li ul.sub-menu li:before { background: $submenu_topborder_color; }";
...
# Page main title 
function dttheme_page_main_title_section($post_id) {
	$tpl_default_settings = get_post_meta($post_id, '_tpl_default_settings', TRUE);
	$tpl_default_settings = is_array($tpl_default_settings) ? $tpl_default_settings : array();

	if (!array_key_exists('show_page_main_title', $tpl_default_settings) ) :
		if(isset($tpl_default_settings['page-main-title-shortcode'])) {
			$page_main_title = do_shortcode($tpl_default_settings['page-main-title-shortcode']);
		}
		$page_main_title = isset($page_main_title) ? $page_main_title : '';
		echo wp_kses($page_main_title, array(
				'h1' => array('class' => array(),'data-delay' => array(),'data-animation' => array()), 
				'h2' => array('class' => array(),'data-delay' => array(),'data-animation' => array()),
				'h3' => array('class' => array(),'data-delay' => array(),'data-animation' => array()),
				'h4' => array('class' => array(),'data-delay' => array(),'data-animation' => array()),
				'h5' => array('class' => array(),'data-delay' => array(),'data-animation' => array()),
				'div' => array('class' => array(),'data-delay' => array(),'data-animation' => array()),
				'h6' => array('class' => array(),'data-delay' => array(),'data-animation' => array())) );
	endif;
}
...
function dttheme_slider_section($post_id, $section) {
	...
	if (array_key_exists('show_slider', $tpl_default_settings) && array_key_exists('slider_type', $tpl_default_settings)) :
		...
		if(is_page_template('tpl-onepage.php')) {
			$op_name = basename( get_permalink($post_id) );
		}
		...
		if($section == 'topsection') {
			if( $tpl_default_settings['slider_type'] != "specialshortcodes" ) {
				...
				if($tpl_header_styles == 'type6') {
					$header_cls = 'below-menu';	
				}
				
				echo '<div class="content '.$header_cls.'" id="'.$op_name.'"><div class="tp-banner-container"><div class="tp-banner"><div id="slider">';
			...
			echo '</div>';
			echo '<div class="dt-sc-hr-invisible-toosmall"></div>';
		}
...
function dttheme_subtitle_section($id=0,$type,$settings = array() ){
	...
	/*	echo '<!-- ** Breadcrumb Section ** -->';
		echo '<div class="main-title dt-sc-breadcrumb-parallax '.$class.'"  style="'.$style.'">';	
...
function dttheme_custom_subtitle_section( $title, $class){
	...
	echo '<div class="main-title '.$class.'">';	
	echo '	<div class="container">';
					if ( is_front_page() && is_home() ) {
	echo "				<h2>".get_bloginfo('description')."</h2>";
					} else {
...
add_filter('comment_reply_link', 'dt_sc_replace_reply_link_class');
function dt_sc_replace_reply_link_class($class){
    $class = str_replace("class='comment-reply-link", "class='comment-reply-link dt-sc-button small", $class);
    return $class;
}