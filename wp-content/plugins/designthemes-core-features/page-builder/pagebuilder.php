		...
		function dtthemes_modules_init(){
			...
			foreach($wp_widget_factory->widgets as $class => $info){
				...
				foreach($selecttag AS $item)
				{
					...
					/* Getting tribe event tags and adding it manually */
					if($prev_class == 'widefat calendar-widget-add-filter') {
						$tribe_terms = get_terms("post_tag");
						foreach($tribe_terms as $term) {
							$tribe_tag_node = $doc->createElement("option", $term->name);
							$tribe_tag_node->setAttribute("value",$term->term_id);
							$item->appendChild($tribe_tag_node);
						}
					}
