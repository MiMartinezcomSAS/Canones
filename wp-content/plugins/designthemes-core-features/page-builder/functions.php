...
if ( ! function_exists('dt_generate_module_options') ){
	function dt_generate_module_options( $module_name, $module_window, $paste_to_editor_id, $dt_module_exact_name, $module_type ){
		...
		if(isset($dtthemes_modules_new[$module_type][$module_name]['options'])):
			...
			echo '<p>'.__('This module don\'t have any attribute to set, just click save changes button to add shortcode.','dt_themes').'</p>';
		endif;
...
function dt_show_columoptions_panel(){
	...
	dt_generate_columnoptions_panel($dt_column_type, $dt_column_css, $dt_column_animation_type, $dt_column_animation_delay);
	
	die();
}

if ( ! function_exists('dt_generate_columnoptions_panel') ){
	function dt_generate_columnoptions_panel( $dt_column_type, $dt_column_css, $dt_column_animation_type, $dt_column_animation_delay ){
		...
		echo '<form id="dt_column_settings">'
			...
			if(/*$enable_animation_effects &&*/ $dt_animation_types != '') {
				...
				echo
				'<p><label>'.__('Custom Animation Type', 'dt_themes').'</label><span class="selection-box"><select name="dt_column_animation_type" id="dt_column_animation_type" >'
					. '<option value="None">  ' . esc_html__('Select', 'dt_themes') . '  </option>';
				...
				echo
				'<p><label>'.__('Custom Animation Delay', 'dt_themes').'</label><input type="number" class="regular-text dtthemes_option" name="dt_column_animation_delay" id="dt_column_animation_delay" value="'.$dt_column_animation_delay.'"  /></p>';
